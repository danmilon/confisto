 ...
 *                              * defaults -- object with default values or string path to defaults.conf
...
function confisto(params, cb) {
  ...
  if (typeof defaults === 'string') {
    fs.readFile(path.resolve(defaults), function (err, defaultsFile) {
      if (err) {
        return cb(err)
      }
      defaults = JSON.parse(defaultsFile)
      doneDefaults()
    })
  }
  else {
  ...
  function doneDefaults() {
    ...
    async.waterfall([
      ...
      function mergeConfig(configs, asyncCb) {
        var mergeArgs = [defaults].concat(configs)
...
var configs = {}
confisto.keep = function (key, value) {
  return (configs[key] = value)
}

confisto.get = function (key) {
  return configs[key]
}
